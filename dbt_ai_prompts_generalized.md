...
>
> **Single client input:** Attach or reference one freeform client requirements document (markdown). The AI extracts domain context, business questions, and schema hints from that document. Nothing else is pre-configured.
...
6. Run Phases 2–6 in sequence. Attach `config.md` + client requirements doc for each phase. Each phase references the approved Design Brief — not hardcoded table lists.
7. Log every run in the **AI Execution Log** at the bottom.

> **Why Phase 1 before Phase 0?** Phase 0 uses `generate_source` and `dbt show`, which require a parsed dbt project, warehouse connection, and installed packages (codegen). Bootstrap creates config only — no models or sources yet.

```mermaid
flowchart TD
    ConfigMd[config.md]
    ClientDoc[ClientRequirements.md]
    Phase1[Phase1_MinimalBootstrap]
    Phase0[Phase0_Discovery_DesignBrief]
    Gate{HumanApproval}
...
## Design Brief Template (Phase 0 Output)
...
```
Read config.md and substitute all project variables. Read the client requirements document at CLIENT_REQUIREMENTS_DOC.
```
...
Do not create _sources.yml or any model SQL. Proceed to Phase 0 after bootstrap succeeds.
```

---

## PHASE 0 — Discovery & Design Brief
...
Read the client requirements document in full. Extract: domain context, business goals, pain points, business questions, and any named source systems or schemas.

Prerequisite check: confirm dbt_project.yml, profiles.yml, and packages (codegen) are installed. If not, run Phase 1 first.
...
5. Propose column standardization (renames, casts, derived flags) using domain language from the client doc — not generic assumptions.
...
6. Draft the full Design Brief (all 10 sections). Include:
   ...
   - semantic metrics mapped to business questions from the client doc
...
**Prerequisite:** Approved Design Brief; Phase 1 bootstrap complete
...
2. dim_{entity}:
   ...
   - Optional segmentation column using CASE logic derived from KPI map and client doc — not hardcoded tiers
   - Materialization: table
...
3. README.md — project runbook:
   - Problem statement from client doc domain summary
...
- Financial formulas not supported by client doc or discovered data

**When to Override AI**

- Any financial calculation — verify formula against client requirements
...
This library uses **dbt codegen only** for schema discovery. When Postgres or Redshift MCP servers are available, an optional Phase 0 supplement can list schemas, tables, and columns via MCP before running `generate_source` — **after Phase 1 bootstrap**. MCP replaces manual schema inspection only, not the approval gate or naming conventions.

```text
Run order with optional MCP (after Phase 1 bootstrap):
  list_schemas → list_tables → list_columns → generate_source (Phase 0) → approve Design Brief → Phases 2–6
```
